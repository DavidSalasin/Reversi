...
public class BitShifters
{
    ...
    static {
        ...
        shiftDict.put(NORTH_WEST, position -> (position << 9) & ~RIGHT_EDGE_MASK);

        // North direction:
        shiftDict.put(NORTH, position -> position << 8);

        // North east direction:
        shiftDict.put(NORTH_EAST, position -> (position << 7) & ~LEFT_EDGE_MASK);

        // east direction:
        shiftDict.put(EAST, position -> (position >>> 1) & ~LEFT_EDGE_MASK);

        // South east direction:
        shiftDict.put(SOUTH_EAST, position -> (position >>> 9) & ~LEFT_EDGE_MASK);

        // south direction:
        shiftDict.put(SOUTH, position -> position >>> 8);

        // south west direction:
        shiftDict.put(SOUTH_WEST, position -> (position >>> 7) & ~RIGHT_EDGE_MASK);

        // west direction:
        shiftDict.put(WEST, position -> (position << 1) & ~RIGHT_EDGE_MASK);
    }
}
